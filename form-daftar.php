...
if (isset($_POST['simpan'])) {
    ...
    if ($nama && $nohp && $pinjaman && $pinjam && $cicilan && $tenggat) {
        if ($op == 'edit') {
            ...
            $q1 = mysqli_query($koneksi, "INSERT INTO customer(nama_cust, no_hp) values ('$nama', '$nohp')");
            $q2 = mysqli_query($koneksi, "INSERT INTO cicilan(jml_pinjaman, jml_cicilan, tgl_pinjam, tgl_tenggat) values ('$pinjaman', '$cicilan', '$pinjam', '$tenggat')");
            $q3 = mysqli_query($koneksi, "INSERT INTO cicilan(id) SELECT id FROM customer");
            $q4 = mysqli_query($koneksi, "INSERT INTO transaksi(id, id_cicilan) select id, id_cicilan FROM cicilan");
            if ($q1 && $q2 && $q3 && $q4) {
                ...
                <table class="table">
                    <th>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    <td scope="row"><?php echo $r2['id_transaksi']; ?></td>
                                    ...
                                    <td scope="row">
                                        <a href="form-daftar.php?op=edit&id_transaksi=<?php echo $id ?>">
                                            <button type="button" class="btn btn-info">Edit</button>
                                        </a>
                                        <a href="form-daftar.php?op=delete&id_transaksi=<?php echo $id ?>" onclick="return confirm('Delete Data?')">
